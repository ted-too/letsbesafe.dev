...
export default function VideoPage() {
  ...
  useEffect(() => {
    ...
    window.open(
      `/action/download?title=${encodeURIComponent(result.title)}&id=${
        result.id
      }&type=${format.format == "mp3" ? "audio" : "video"}&format=${
        format.format
      }&quality=${format.quality.replace("p", "")}`,
      "_blank"
    );
  }, [format]);
  return (
      ...
      {!result ? (
        ...
        <div className="flex justify-between w-full max-w-4xl mx-auto mb-16">
          <div className="flex flex-col space-y-4 w-80 rounded-md">
            ...
            <span className="text-lg text-normal font-semibold line-clamp-3">
              {result.title}
            </span>
          </div>
          <FormatSelector
