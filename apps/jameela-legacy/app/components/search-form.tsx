...
export default function SearchForm({ firstSearch = false, children = null }: Props) {
  ...
  return (
    <>
      ...
      <fetcher.Form method="post" action="/action/search" className="relative w-full max-w-sm sm:max-w-none">
        <fieldset
        ...
        >
          <TextInput
            ...
            className="w-full sm:w-[50%] lg:w-[40%]"
          ...
          <Button
            ...
            className="text-sm rounded-lg sm:text-base"
          >
            Search
          </Button>
        </fieldset>
      ...
      <div className="relative w-full transition-opacity max-w-7xl">{children}</div>
    </>
  );
}
...
export const ErrorBoundary: ErrorBoundaryComponent = ({ error }) => {
  return (
    <div className="mt-[10vh] mx-6 w-full sm:max-w-xl max-w-3xl bg-pink-50 dark:bg-pink-500 border border-pink-400 text-normal rounded-lg p-10 pb-14 sm:pb-28 flex flex-col items-center">
      <svg className="w-auto h-12 mb-6 sm:h-24" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
      ...
      <span className="text-2xl font-semibold text-center sm:text-3xl">Oh no!</span>
      <span className="text-lg font-light text-center sm:text-xl">Something went wrong</span>
      <p className="p-4 mt-4 text-center bg-white">{error.message}</p>
    </div>
  );
};
